...
```sh
make docker-build docker-push IMG=<some-registry>/my-computeclass:tag
```
...
```sh
make deploy IMG=<some-registry>/my-computeclass:tag
```
...
```sh
make build-installer IMG=<some-registry>/my-computeclass:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/my-computeclass/<tag or branch>/dist/install.yaml
```
